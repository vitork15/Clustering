...
class ARFCM_ER:
    ...
    def objective_function(self):
        ...
        # distance component
        c_diff = self.data[:, np.newaxis, :, 0] - self.prototype_vector[np.newaxis, :, :, 0]
        r_diff = self.data[:, np.newaxis, :, 1] - self.prototype_vector[np.newaxis, :, :, 1]

        lower_components = ((c_diff - r_diff) ** 2) * self.lower_weight_matrix[np.newaxis, :, :]
        upper_components = ((c_diff + r_diff) ** 2) * self.upper_weight_matrix[np.newaxis, :, :]

        value += np.sum(self.membership_matrix * np.sum(lower_components + upper_components, axis=2))

        # entropy component
        not_zero = self.membership_matrix > 0
        value += self.T_u * np.sum(self.membership_matrix[not_zero] * np.log(self.membership_matrix[not_zero]))

        return value
    
    def minimum_centroid_distance(self):

        c_diff = self.prototype_vector[:, np.newaxis, :, 0] - self.prototype_vector[np.newaxis, :, :, 0]
        r_diff = self.prototype_vector[:, np.newaxis, :, 1] - self.prototype_vector[np.newaxis, :, :, 1]
    ...
    def run(self, data, cluster_num, T_u=1, max_iterations=200, threshold=1e-10):
        ...
        data_num = data.shape[0]
        self.prototype_vector = []
        self.upper_weight_matrix = np.ones((cluster_num, feature_num))
        self.lower_weight_matrix = np.ones((cluster_num, feature_num))
        self.membership_matrix = np.zeros((data_num, cluster_num))
        self.adequacy_history = []
        self.T_u = T_u
        ...
        while t < max_iterations:
            ...
            # fuzzy cluster prototype computation (representation step)
            weight_sums = np.sum(self.membership_matrix, axis=0)
            valid_clusters = weight_sums > 1e-10
            if np.any(valid_clusters):
                weighted_sum_c = self.membership_matrix.T @ data[:, :, 0]
                weighted_sum_r = self.membership_matrix.T @ data[:, :, 1]

                self.prototype_vector[valid_clusters, :, 0] = weighted_sum_c[valid_clusters] / weight_sums[valid_clusters, np.newaxis]
                self.prototype_vector[valid_clusters, :, 1] = weighted_sum_r[valid_clusters] / weight_sums[valid_clusters, np.newaxis]

            # width parameter computation (weighting step)
            c_diff = self.data[:, np.newaxis, :, 0] - self.prototype_vector[np.newaxis, :, :, 0]
            r_diff = self.data[:, np.newaxis, :, 1] - self.prototype_vector[np.newaxis, :, :, 1]

            lower_components = np.sum(((c_diff - r_diff) ** 2) * self.membership_matrix[:, :, np.newaxis], axis=0)
            upper_components = np.sum(((c_diff + r_diff) ** 2) * self.membership_matrix[:, :, np.newaxis], axis=0)
    ...
    def run_many(self, data, cluster_num, T_u=1, max_iterations=50, reinitializations=30, threshold=1e-5):
        ...
        while reinitializations > 0:
            trial_run = self.run(data, cluster_num, T_u, max_iterations, threshold)
            if best_run.adequacy_history[-1] > trial_run.adequacy_history[-1]:
                best_run = trial_run
            reinitializations = reinitializations - 1
